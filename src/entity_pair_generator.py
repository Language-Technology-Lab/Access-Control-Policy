"""
Entity Pair Generator for Relation Classification Testing

This module provides functions to generate test entity pairs for relation classification
based on different relation types and their specific rules.

The module supports three relation types with distinct generation rules:
...
USAGE EXAMPLES:
    ...
    from src.entity_pair_generator import (
        generate_all_relation_triples, get_triple_statistics,
        parse_predicted_entities, parse_ground_truth_entities
    )

    # Load your entity and ground truth data
    predicted_entities = load_json('path/to/predicted_entities.json')
    ground_truth_data = load_json('path/to/ground_truth.json')

    # Direct parsing examples:
    user_attrs, obj_attrs, policies = parse_predicted_entities(predicted_entities)
    user_attrs, obj_attrs, policies = parse_ground_truth_entities(ground_truth_data)

    # Generate all relation triples for testing
    # For predicted entities format:
    triples = generate_all_relation_triples(predicted_entities, ground_truth_data, entities_source="predicted")

    # For ground truth entities format:
    triples = generate_all_relation_triples(ground_truth_data, ground_truth_data, entities_source="ground_truth")

    # Auto-detect format (recommended):
    triples = generate_all_relation_triples(predicted_entities, ground_truth_data)  # entities_source="auto"

    # Get statistics about the generated triples
    stats = get_triple_statistics(triples)
    print(f"Generated {stats['total_triples']} test triples")
    print(f"Assign: {stats['assign_triples']} ({stats['positive_assign']} positive)")
    print(f"Permit: {stats['permit_triples']} ({stats['positive_permit']} positive)")
    print(f"Prohibit: {stats['prohibit_triples']} ({stats['positive_prohibit']} positive)")
"""
